    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-background via-primary/5 to-game-purple/5 p-4">
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          {/* Requirements Created by Team */}
          <TeamRequirementsSection teamId={selectedTeam.id} />

          {/* Evaluations Made by Team */}
          <TeamEvaluationsSection teamId={selectedTeam.id} />
...
// Component to show requirements created by the team
function TeamRequirementsSection({ teamId }: { teamId: string }) {
  const [requirements, setRequirements] = useState<any[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    loadTeamRequirements()
  }, [teamId])

  const loadTeamRequirements = async () => {
    try {
      const [{ data: reqs, error: reqError }, { data: participants }] = await Promise.all([
        supabase
          .from('requirements')
          .select('*')
          .eq('session_id', teamId)
          .order('created_at', { ascending: false }),
        supabase
          .from('session_participants')
      ...
      for (const p of participants || []) nameById[p.id] = p.player_name

      setRequirements(
        (reqs || []).map(r => ({ ...r, creator_name: nameById[r.created_by] || 'Desconhecido' }))
      )
    } catch (error) {
  ...
  if (loading) {
    return (
      <GameCard className="space-y-4">
        <h3 className="font-semibold">Requisitos Criados pela Equipe</h3>
  ...
  return (
    <GameCard className="space-y-4">
      <h3 className="font-semibold">Requisitos Criados pela Equipe ({requirements.length})</h3>
      {requirements.length === 0 ? (
        <p className="text-center py-4 text-muted-foreground">Nenhum requisito encontrado</p>
      ) : (
        <div className="space-y-3">
...
// Component to show evaluations made by the team
function TeamEvaluationsSection({ teamId }: { teamId: string }) {
  ...
  const loadTeamEvaluations = async () => {
    try {
      const [{ data: evals, error: evalError }, { data: participants }, { data: reqs }] = await Promise.all([
        supabase
          .from('requirement_evaluations')
          .select('*')
          .eq('session_id', teamId)
      ...
      const reqById: Record<string, { requirement_text: string; classification: string }> = {}
      for (const r of reqs || []) reqById[r.id] = { requirement_text: r.requirement_text, classification: r.classification }

      setEvaluations(
        (evals || []).map(e => ({
          ...e,
          evaluator_name: nameById[e.evaluator_id] || 'Desconhecido',
          requirement: reqById[e.requirement_id] || null,
        }))
      )
    } catch (error) {
  ...
  if (loading) {
    return (
      <GameCard className="space-y-4">
        <h3 className="font-semibold">Avaliações Feitas pela Equipe</h3>
        <div className="text-center py-4">
          <RefreshCw className="w-6 h-6 animate-spin mx-auto mb-2" />
  ...
  return (
    <GameCard className="space-y-4">
      <h3 className="font-semibold">Avaliações Feitas pela Equipe ({evaluations.length})</h3>
      {evaluations.length === 0 ? (
        <p className="text-center py-4 text-muted-foreground">Nenhuma avaliação encontrada</p>
      ) : (
        <div className="space-y-3">
          {evaluations.map((evaluation) => {
            const classificationInfo = CLASSIFICATIONS.find(c => c.id === evaluation.requirement?.classification)
            return (
              <div key={evaluation.id} className="p-4 border rounded-lg bg-muted/20">
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant={evaluation.is_correct ? "default" : "destructive"} className="text-xs">
                      {evaluation.is_correct ? "Correta" : "Incorreta"}
                    </Badge>
                    <Badge className={classificationInfo?.color || "bg-gray-100 text-gray-800"}>
                      {classificationInfo?.label || evaluation.requirement?.classification}
                    </Badge>
                  </div>
                  <div className="text-right text-xs text-muted-foreground">
                    <div>Por: {evaluation.evaluator_name}</div>
                  </div>
                </div>

                <div className="space-y-2">
                  <div>
                    <p className="text-sm font-medium">Requisito avaliado:</p>
                    <p className="text-sm text-muted-foreground bg-muted/30 p-2 rounded">
                      {evaluation.requirement?.requirement_text || 'Texto não encontrado'}
                    </p>
                  </div>
                ...
                <div className="mt-3 pt-2 border-t text-xs text-muted-foreground">
                  Avaliado em: {new Date(evaluation.created_at).toLocaleDateString('pt-BR')} às {' '}
                  {new Date(evaluation.created_at).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}
                </div>
              </div>
            )
          })}
